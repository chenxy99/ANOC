...
from updown.data.datasets_penultimate_saliency import (
    ...
    TrainingEvaluationDatasetWithConstraintsAndPenultimateSaliencySCST,
...
from updown.models import UpDownSaliencyCaptioner, UpDownSaliencyCaptionerSCST
...
if __name__ == "__main__":
    ...
    train_dataset = TrainingEvaluationDatasetWithConstraintsAndPenultimateSaliencySCST.from_config(_C, vocabulary=vocabulary, in_memory=_A.in_memory)

    # Use a smaller batch during SCST training (accounting beam size) to fit in memory.
    scst_train_batch_size = _C.OPTIM.BATCH_SIZE // _C.MODEL.BEAM_SIZE

    # Reduce batch size by total FSM states during CBS, because net beam size is larger.
    if _C.MODEL.USE_CBS:
        scst_train_batch_size = scst_train_batch_size // (2 ** _C.DATA.CBS.MAX_GIVEN_CONSTRAINTS)
        scst_train_batch_size = scst_train_batch_size or 1

    train_dataloader = DataLoader(
        train_dataset,
        batch_size=scst_train_batch_size,
        shuffle=True,
    ...
    model = UpDownSaliencyCaptionerSCST.from_config(_C, vocabulary=vocabulary).to(device)
    ...
    if _A.start_from_checkpoint != "":
        training_checkpoint: Dict[str, Any] = torch.load(_A.start_from_checkpoint, map_location={'cuda:0': 'cpu', 'cuda:1': 'cpu'})
    ...
    cider_train = Cider(PTBTokenizer.tokenize(train_dataset._captions_reader._ref_caps_full_sentences))
    ...
    for iteration in tqdm(range(start_iteration, _C.OPTIM.NUM_ITERATIONS + 1)):
        ...
        batch_log_probabilities = output_dict["log_probabilities"].view(-1, _C.MODEL.BEAM_SIZE)
        batch_valid_num = output_dict["valid_numbers"]
        ...
        for i in range(batch_predictions.shape[0]):
            ...
            # )
            predictions[i] = [" ".join(caption)]

        gts: Dict[int, List[str]] = {}
        counter = 0
        for instance, number in zip(batch["captions"], batch_valid_num):
            for index in range(_C.MODEL.BEAM_SIZE * number):
                gts[counter] = instance
                counter += 1

        reward = cider_train.compute_score(gts, predictions)[1].astype(np.float32)

        reward = torch.from_numpy(reward).to(batch["image_features"].device).view(-1, _C.MODEL.BEAM_SIZE)
        reward_baseline = torch.mean(reward, -1, keepdim=True)
        # print(reward_baseline)
        loss = -batch_log_probabilities * (reward - reward_baseline)
        ...
        reward_counter += 1
        mean_reward = running_reward / reward_counter
        ...
        tensorboard_writer.add_scalar("mean_reward", mean_reward, iteration)
