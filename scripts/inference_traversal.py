...
parser.add_argument(
    "--tensorboard-log-path", required=True, help="Path to record the evaluation results."
)
...
if __name__ == "__main__":
    ...
    tensorboard_writer = SummaryWriter(logdir=_A.tensorboard_log_path)
    ...
    for checkpoint_index in range(30000, 71000, 1000):
        checkpoint_index_str = str(checkpoint_index)
        full_checkpoint_path = os.path.join(_A.checkpoint_path, "checkpoint_" + checkpoint_index_str + ".pth")
        ...
        model.load_state_dict(torch.load(full_checkpoint_path)["model"])
        ...
        if _A.evalai_submit:
            ...
            print(f"Evaluation metrics after iteration {checkpoint_index}:")
            for metric_name in evaluation_metrics:
                tensorboard_writer.add_scalars(
                    f"metrics/{metric_name}", evaluation_metrics[metric_name], checkpoint_index
                )
                print(f"\t{metric_name}:")
