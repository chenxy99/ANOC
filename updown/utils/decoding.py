...
def select_valid_beam_with_constraints(
    beams: torch.Tensor,
    beam_log_probabilities: torch.Tensor,
    given_constraints: torch.Tensor,
    min_constraints_to_satisfy: int = 2,
    ...
    valid_beams: List[torch.Tensor] = []
    valid_beam_log_probabilities: List[torch.Tensor] = []
    valid_beams_num: List[int] = []
    ...
    for i in range(batch_size):
        ...
        _valid_beams = beams[i, valid_states, :, :].view(-1, max_decoding_steps)
        _valid_beam_log_probabilities = beam_log_probabilities[i, valid_states, :].view(-1)

        # top_k = valid_beam_log_probabilities.view(-1).argsort(descending=True)[:beam_size]
        # topk_valid_beams = valid_beams.view(-1, max_decoding_steps)[top_k]
        # topk_valid_beam_log_probabilities = valid_beam_log_probabilities.view(-1)[top_k]

        for index in range(len(valid_states) * beam_size):
            valid_beams.append(_valid_beams[index])
            valid_beam_log_probabilities.append(_valid_beam_log_probabilities[index])
        valid_beams_num.append(len(valid_states))
        # selected_index = torch.argmax(valid_beam_log_probabilities)
        # best_beams.append(valid_beams[selected_index, :])
        # best_beam_log_probabilities.append(valid_beam_log_probabilities[selected_index])

    # shape: (batch_size, max_decoding_steps)
    return torch.stack(valid_beams).long().to(beams.device), \
           torch.stack(valid_beam_log_probabilities).to(beams.device), \
           torch.tensor(valid_beams_num).long().to(beams.device)
