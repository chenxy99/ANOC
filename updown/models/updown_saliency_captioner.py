...
class UpDownSaliencyCaptioner(nn.Module):
    r"""
    ...
    def forward(  # type: ignore
        ...
        if self.training and caption_tokens is not None:
            ...
            step_combined_logits: List[torch.Tensor] = []
            for timestep in range(num_decoding_steps):
                # shape: (batch_size,)
                input_tokens = caption_tokens[:, timestep]

                # shape: (batch_size, num_classes)
                output_logits, output_logits_saliency, combined_output_logits, states = \
                ...
                # list of tensors, shape: (batch_size, 1, vocab_size)
                step_combined_logits.append(combined_output_logits.unsqueeze(1))
            ...
            # shape: (batch_size, num_decoding_steps)
            combined_logits = torch.cat(step_combined_logits, 1)
            ...
            output_dict = {
                ...
                "combined_loss": self._get_loss(
                    combined_logits, caption_tokens[:, 1:].contiguous(), tokens_mask[:, 1:].contiguous(),
                )
            }
        else:
