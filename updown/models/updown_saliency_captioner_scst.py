...
from updown.modules import UpDownCell, UpDownSaliencyCell, ConstrainedBeamSearch
from updown.utils.decoding import (select_best_beam, select_best_beam_with_constraints,
                                   select_topk_beam_with_constraints, select_valid_beam_with_constraints)


class UpDownSaliencyCaptionerSCST(nn.Module):
    r"""
    Image captioning model using bottom-up top-down attention, as similar in
    ...
    runs :class:`~updown.modules.updown_saliency_cell.UpDownSaliencyCell` for multiple time-steps. If this class is
    analogous to an :class:`~torch.nn.LSTM`, then :class:`~updown.modules.updown_saliency_cell.UpDownSaliencyCell`
    ...
    def __init__(
        ...
        self._updown_saliency_cell = UpDownSaliencyCell(
            image_feature_size, embedding_size, hidden_size, attention_projection_size, penultimate_feature_size,
        ...
        if self.embedding_size == 300:
            ...
            self._output_layer_saliency = nn.Linear(self.embedding_size, _vocab_size, bias=False)
    ...
    def from_config(cls, config: Config, **kwargs):
        ...
        return cls(
            ...
            penultimate_feature_size=_C.MODEL.PENULTIMATE_FEATURE_SIZE,
            saliency_attention_projection_size=_C.MODEL.SALIENCY_ATTENTION_PROJECTION_SIZE,
    ...
    def forward(  # type: ignore
        ...
        penultimate_features = penultimate_features.view(batch_size, channel, -1).transpose(1, 2).contiguous()
        ...
        if self.training and caption_tokens is not None:
            ...
            for timestep in range(num_decoding_steps):
                ...
                # list of tensors, shape: (batch_size, 1, vocab_size)
                step_logits_saliency.append(output_logits_saliency.unsqueeze(1))
            ...
            output_dict = {
                ...
                "loss_saliency": self._get_loss(
                    logits_saliency, caption_tokens[:, 1:].contiguous(), tokens_mask[:, 1:].contiguous(),
                )
            }
        else:
            ...
            beam_decode_step = functools.partial(self._decode_step, image_features, penultimate_features)
            ...
            if self._is_val:
                ...
                output_dict = {"predictions": valid_beam, "log_probabilities": valid_log_probabilities,
    ...
    def _decode_step(
        ...
        penultimate_features: torch.Tensor
            A tensor of shape ``(batch_size, width * height, image_feature_size)``.
        previous_predictions: torch.Tensor
        ...
        if not self.training and image_features.size(0) != previous_predictions.size(0):
            ...
            batch_size, num_boxes, image_feature_size = image_features.size()
            batch_size, penultimate_nums, penultimate_size = penultimate_features.size()
            ...
            # Add (net) beam dimension and repeat penultimate features.
            penultimate_features = penultimate_features.unsqueeze(1).repeat(1, net_beam_size, 1, 1)
            ...
            # shape: (batch_size * net_beam_size, width*height, penultimate_feature_size)
            penultimate_features = penultimate_features.view(
                batch_size * net_beam_size, penultimate_nums, penultimate_size
            )
        ...
        updown_saliency_output, updown_saliency_output_saliency, states, gate \
            = self._updown_saliency_cell(image_features, penultimate_features, token_embeddings, states)

        # shape: (batch_size * net_beam_size, vocab_size)
        updown_saliency_output = self._output_projection(updown_saliency_output)
        output_saliency_logits = self._output_layer(updown_saliency_output)

        # shape: (batch_size * net_beam_size, vocab_size)
        updown_saliency_output_saliency = self._output_projection_saliency(updown_saliency_output_saliency)
        output_saliency_logits_saliency = self._output_layer_saliency(updown_saliency_output_saliency)
        ...
        outputs_logits = torch.log(gate * self._softmax(output_saliency_logits)
                                   + (1 - gate) * self._softmax(output_saliency_logits_saliency) + 1e-20)
        if self.training:
            return output_saliency_logits, output_saliency_logits_saliency, outputs_logits, states
        else:
            # outputs_logits = torch.log(gate * self._softmax(output_saliency_logits)
            #                            + (1 - gate) * self._softmax(output_saliency_logits_saliency) + 1e-20)
            # outputs1 = torch.log(_lambda * self._softmax(output_saliency_logits)
            #                     + (1 - _lambda) * self._softmax_saliency(output_saliency_logits_saliency) )
            # if self._is_val:
            #     outputs = torch.log(_lambda * self._softmax(output_saliency_logits)
            #                         + (1 - _lambda) * self._softmax_saliency(output_saliency_logits_saliency))
            # else:
            #     if random.random() <= 0.5:
            #         outputs = self._log_softmax(output_saliency_logits)
            #     else:
            #         outputs = self._log_softmax(output_saliency_logits_saliency)
            return outputs_logits, states
        # outputs = output_saliency_logits if self.training else self._log_softmax(output_saliency_logits)
